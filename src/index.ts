...
const ACCESS_LOG_ENABLED = parseBooleanEnv((process.env.ACCESS_LOG_ENABLED ?? "").trim().toLowerCase()) ?? true;
const ACCESS_LOG_FILE = path.resolve(process.cwd(), "_sync", "logs", "access.log");
const SSE_HEARTBEAT_MS = Math.max(5000, Math.min(60000, Number(process.env.SSE_HEARTBEAT_MS ?? "15000") || 15000));
const MCP_STREAM_HEARTBEAT_MS = Math.max(5000, Math.min(60000, Number(process.env.MCP_STREAM_HEARTBEAT_MS ?? "15000") || 15000));
const MCP_STREAM_MAX_MS = Math.max(5000, Math.min(300000, Number(process.env.MCP_STREAM_MAX_MS ?? "12000") || 12000));
...
let accessLogInitPromise: Promise<void> | null = null;

function ensureAccessLogPath(): Promise<void> {
  if (!ACCESS_LOG_ENABLED) return Promise.resolve();
  if (!accessLogInitPromise) {
    accessLogInitPromise = fs.mkdir(path.dirname(ACCESS_LOG_FILE), { recursive: true }).then(() => undefined);
  }
  return accessLogInitPromise;
}

function writeAccessLog(entry: Record<string, any>) {
  if (!ACCESS_LOG_ENABLED) return;
  const line = `${JSON.stringify(entry)}\n`;
  void ensureAccessLogPath()
    .then(() => fs.appendFile(ACCESS_LOG_FILE, line, "utf8"))
    .catch((error) => {
...
function toFetchRequest(req: any, signal?: AbortSignal) {
...
async function sendFetchResponseToExpress(webRes: Response, res: any, rpcMethod?: string) {
  ...
  const heartbeat = isSse
    ? setInterval(() => {
        if (closed) return;
        try {
          res.write(`: ping ${Date.now()}\n\n`);
        } catch {
          // Ignore write failures; close lifecycle handles cleanup.
        }
...
`;
  ...
  async function runSearchWeb(args: any) {
    ...
    const extractDdgsHtmlResults = (html: string): Array<{ text: string; url?: string }> => {
      const out: Array<{ text: string; url?: string }> = [];
      if (!html) return out;
      const regex = /<a[^>]*class="[^"]*result__a[^"]*"[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi;
      let match: RegExpExecArray | null;
      while ((match = regex.exec(html)) !== null && out.length < maxResults) {
        const hrefRaw = String(match[1] ?? "");
        const textRaw = String(match[2] ?? "")
          .replace(/<[^>]+>/g, " ")
          .replace(/&amp;/g, "&")
          .replace(/&quot;/g, "\"")
          .replace(/&#39;/g, "'")
        ...
        out.push({ text: textRaw, url: href });
      }
      return out;
    };
    const extractBingHtmlResults = (html: string): Array<{ text: string; url?: string }> => {
      const out: Array<{ text: string; url?: string }> = [];
      if (!html) return out;
      const regex = /<li[^>]*class="[^"]*b_algo[^"]*"[\s\S]*?<a[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi;
      let match: RegExpExecArray | null;
      while ((match = regex.exec(html)) !== null && out.length < maxResults) {
        ...
        const textRaw = String(match[2] ?? "")
          ...
          .trim();
        if (!hrefRaw || !textRaw) continue;
        if (!/^https?:\/\//i.test(hrefRaw)) continue;
        out.push({ text: textRaw, url: hrefRaw });
      }
      return out;
    };
    ...
    try {
      ...
      providersTried.push("duckduckgo_instant");
      const res = await fetch(url, { signal: controller.signal, headers: browserHeaders });
      ...
      if (flat.length > 0 || String(json?.AbstractText ?? "").trim() || String(json?.Answer ?? "").trim()) {
        providerUsed = "duckduckgo_instant";
      }
      if (flat.length === 0 && !String(json?.AbstractText ?? "").trim() && !String(json?.Answer ?? "").trim()) {
        try {
          providersTried.push("duckduckgo_html");
          const htmlUrl = `https://html.duckduckgo.com/html/?q=${encodeURIComponent(query)}`;
          const htmlRes = await fetch(htmlUrl, { signal: controller.signal, headers: browserHeaders });
          const htmlRaw = await htmlRes.text();
          if (htmlRes.ok) {
            const parsedHtml = extractDdgsHtmlResults(htmlRaw);
            if (parsedHtml.length > 0) {
              flat.push(...parsedHtml.slice(0, maxResults));
              providerUsed = "duckduckgo_html";
            }
          }
        } catch {
          // Keep original IA result if HTML fallback fails.
        }
      }
      if (flat.length === 0 && !String(json?.AbstractText ?? "").trim() && !String(json?.Answer ?? "").trim()) {
        try {
          providersTried.push("bing_html");
          const bingUrl = `https://www.bing.com/search?q=${encodeURIComponent(query)}&setlang=en-US`;
          const bingRes = await fetch(bingUrl, { signal: controller.signal, headers: browserHeaders });
          const bingRaw = await bingRes.text();
          if (bingRes.ok) {
            const parsedBing = extractBingHtmlResults(bingRaw);
            if (parsedBing.length > 0) {
              flat.push(...parsedBing.slice(0, maxResults));
              providerUsed = "bing_html";
            }
          }
        } catch {
      ...
      if (flat.length === 0 && !String(json?.AbstractText ?? "").trim() && !String(json?.Answer ?? "").trim()) {
        try {
          providersTried.push("wikipedia_opensearch");
          const wikiUrl =
          ...
          const wikiRes = await fetch(wikiUrl, { signal: controller.signal, headers: browserHeaders });
          const wikiRaw = await wikiRes.text();
          if (wikiRes.ok) {
            const wikiJson = wikiRaw ? JSON.parse(wikiRaw) : [];
            const titles = Array.isArray(wikiJson?.[1]) ? wikiJson[1] : [];
            const descs = Array.isArray(wikiJson?.[2]) ? wikiJson[2] : [];
            const links = Array.isArray(wikiJson?.[3]) ? wikiJson[3] : [];
            for (let i = 0; i < titles.length && flat.length < maxResults; i += 1) {
            ...
            if (flat.length > 0) {
              providerUsed = "wikipedia_opensearch";
            }
          }
        } catch {
          // Keep best-effort behavior; do not fail hard on fallback.
        }
      }
      if (!providerUsed) {
        providerUsed = "none";
  ...
  async function runReadUrlContent(args: any) {
    ...
    try {
      ...
      try {
        finalRes = await fetch(parsed.toString(), { signal: controller.signal, headers: browserHeaders });
        raw = await finalRes.text();
        if (!finalRes.ok && [401, 403, 429].includes(finalRes.status)) {
          shouldTryProxy = true;
        }
      } catch {
        shouldTryProxy = true;
      ...
      if (shouldTryProxy) {
        try {
          const proxyUrl = `https://r.jina.ai/http://${parsed.toString().replace(/^https?:\/\//, "")}`;
          const proxyRes = await fetch(proxyUrl, { signal: controller.signal, headers: browserHeaders });
          const proxyRaw = await proxyRes.text();
          if (proxyRes.ok && proxyRaw.trim().length > 0) {
            finalRes = proxyRes;
            raw = proxyRaw;
            usedProxy = true;
          }
        } catch {
          // Keep original error below if proxy fallback fails.
        }
      }
      if (!finalRes || !finalRes.ok) {
        if (finalRes) {
          throw new Error(`read_url_content HTTP ${finalRes.status}: ${raw.slice(0, 300)}`);
        }
        throw new Error("read_url_content fetch failed");
      }
      const normalized = raw.replace(/\r\n/g, "\n");
      return {
        url: parsed.toString(),
        final_url: finalRes.url || parsed.toString(),
        status: finalRes.status,
        content_type: String(finalRes.headers.get("content-type") ?? ""),
...
app.use((req, res, next) => {
  ...
  const ip = String(req.headers['cf-connecting-ip'] || forwardedFor || '');
  let done = false;
  function log(tag: string) {
    ...
    const extra =
      ...
      (ray ? ' ray=' + ray : '') +
      (ua ? ' ua=' + ua : '');
    const message = tag + ' #' + rid + ' ' + req.method + ' ' + url + ' -> ' + res.statusCode + ' (' + ms + 'ms)' + extra;
    console.log(message);
    writeAccessLog({
      ts: new Date().toISOString(),
      event: tag,
...
app.all("/mcp", async (req, res) => {
  const controller = new AbortController();
  const onRequestCanceled = () => controller.abort("client_disconnected");
  req.on("aborted", onRequestCanceled);
  req.on("close", onRequestCanceled);
  const streamRotateTimer = req.method === "GET"
    ? setTimeout(() => {
        ...
        try {
          ...
          console.error("[GET] /mcp stream rotation end failed:", error);
        }
      }, MCP_STREAM_MAX_MS)
    : null;
  ...
  try {
    ...
    // Enforce SSE accept only for stream GET requests.
    if (req.method === "GET" && !acceptHeader.toLowerCase().includes("text/event-stream")) {
      res.status(406).json({
        jsonrpc: "2.0",
...
const webRes = await transport.handleRequest(fetchReq, opts as any);
    ...
    if (streamRotateTimer) clearTimeout(streamRotateTimer);
    req.off("aborted", onRequestCanceled);
    req.off("close", onRequestCanceled);
  }
});
...
app.get("/sse-simple", async (req, res) => {
  ...
  try {
    ...
    // Heartbeat keeps intermediaries from timing out idle SSE streams.
...
const httpServer = app.listen(PORT, () => {
  ...
  console.log(`Access log: ${ACCESS_LOG_ENABLED ? ACCESS_LOG_FILE : "disabled"}`);
  console.log(`SSE heartbeat interval: ${SSE_HEARTBEAT_MS}ms`);
  console.log(`MCP stream heartbeat interval: ${MCP_STREAM_HEARTBEAT_MS}ms`);
  console.log(`MCP stream max lifetime: ${MCP_STREAM_MAX_MS}ms`);
